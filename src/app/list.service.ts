...
export class ListService {
  ...
  */
  moveItemToOtherList(itemIndex:number, listFromIndex: number, listToIndex:number){
    let itemAMover = this.list[listFromIndex].splice(itemIndex, 1);
    //Como hemos hecho un splice itemAMover será otro array, por eso usamos concat y no push
    this.list[listToIndex] = this.list[listToIndex].concat(itemAMover);
    
    //Guardamos las dos listas
    this.saveList(listFromIndex);
    this.saveList(listToIndex);
  }
}
